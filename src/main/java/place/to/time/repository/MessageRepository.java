...
public interface MessageRepository extends JpaRepository<Message, Long> {
    @Query("SELECT m FROM Message m WHERE (m.userNameFrom = :userNameFrom OR m.userNameFrom = :userNameTo) AND (m.userNameTo = :userNameTo OR m.userNameTo = :userNameFrom) ORDER BY m.messageTime ASC ")
